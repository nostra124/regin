...
pub const SETTINGS: &[(&str, &str, &str)] = &[
    ...
    ("llm.base_url", "", "Generic OpenAI-compatible LLM endpoint — when set, used instead of the Mimir gateway (FEAT-083)"),
    ("llm.api_key", "", "API key for the generic OpenAI-compatible endpoint, sent as Authorization: Bearer <key> (FEAT-083)"),
    ("llm.model", "auto", "Model for the generic OpenAI-compatible endpoint (FEAT-083)"),
